...
f = open('KMdata1.txt','wb')
f.write('generation,train,validation,test\n')
f2 = open('features1.txt','wb')
iteration = 0
...
while (avrR2 < 0.85 and iteration < 30):
	...
	model.compile(loss='mean_squared_error', optimizer='sgd')
	model.fit(features2013[[class_[0][i],class_[1][j],
			class_[2][k],class_[3][l],
			class_[4][m],class_[5][n],
			class_[6][p],class_[7][q],
			class_[8][s],class_[9][t]]].values,target2013.values, nb_epoch=5, batch_size=5)
	pre_ = model.predict_proba(features2014[[class_[0][i],class_[1][j],
			class_[2][k],class_[3][l],
			class_[4][m],class_[5][n],
			class_[6][p],class_[7][q],
			class_[8][s],class_[9][t]]].values,batch_size=5)
